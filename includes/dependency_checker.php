...
if (!file_exists($thumbnails_dir)) {
    if(!mkdir($thumbnails_dir)) {
        $html = '<p>'.$translator->string('The <b>thumbnails</b> directory was missing, and could not be created. Please make sure the installation path is writable. As a minimum the <b>gallery</b> and <b>thumbnail</b> directories should be writable.') .'</p>';
        $html .= '<p>'. $translator->string('If using Apache on debian/Ubuntu based systems:') . '</p>';
        $html .= '<pre style="padding:0.5rem;margin-top:1rem;">sudo chmod 775 -R /var/www/my-site-name' . "\n";
        $html .= 'sudo chown www-data:www-data -R /var/www/my-site-name</pre>';
        respond(500, $html);
